...
def run_engine():
    try:
        ...
        Write a 1,000-word authority guide. Tone: Expert, production-level experience (use "I/We").
        Content Rules:
        1. Hook: Problem-First reflection.
        2. Foundation: Link to: "{recent_topic}" using: {link_strat}.
        3. Proof: Cite: {proof}.
        4. CTA: Insert decision-stage CTA: {cta}.
        ...
        def process_match(match):
            ...
            style = f"{prompt}, professional technical schematic, labeled components, large clear text-callouts, vector illustration, white background, color palette #2563eb and #1e293b, minimalist UI documentation style, 8k"
            url = f"https://image.pollinations.ai/prompt/{urllib.parse.quote(style)}?width=1000&height=560&nologo=true"
            return f'<div class="img-container"><img src="{url}" loading="lazy"><div class="caption">{prompt}</div></div>'
        
        html_content = re.sub(r'\[AI_IMAGE:\s*(.*?)\]', process_match, resp2.text)
        
        # Save History & Deploy (Logic simplified for brevity)
